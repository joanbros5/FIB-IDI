...
uniform vec3 posFocus; //Foco de escena
uniform vec3 posFocus2; //Foco de cámara

//Booleano para saber si es foco de escena
uniform int focusEscena;

//Uniforms de colorFocus
...
{	
	...
	//Calculo de focus (El original, de escena)
    vec4 focusSCO;
    ...
    //Calculo de focus nuevo (Siempre de cámara)
    vec4 focus2SCO = vec4(posFocus2,1.0);

    vec3 LSCO2 = focus2SCO.xyz-vertexSCO.xyz;

    vec3 L2 = normalize(LSCO2);
    
    //Color final (Sumando los dos focos)
    vec3 fcolor = Ambient() + Difus(NormSCOF, L, colorFocus) + Especular(NormSCOF, L, vertexSCO, colorFocus) + Difus(NormSCOF, L2, colorFocus2) + Especular(NormSCOF, L2, vertexSCO, colorFocus2);
